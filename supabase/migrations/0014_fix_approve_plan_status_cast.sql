-- Fix: the status CASE returns text, which must be cast to objective_status
-- before assignment (the difficulty cast was already present).
...
begin
  ...
  loop
    ...
    set difficulty  = (v_edit->>'difficulty')::difficulty,
        ...
        status      = (case when v_included and v_order = 0 then 'current' else 'upcoming' end)::objective_status
